...
class CBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        global init_value
        logger.info(f"Use init_values: {init_value}")
        self.gamma_1 = nn.Parameter(init_value * torch.ones((1, dim, 1, 1, 1)),requires_grad=True)
        self.gamma_2 = nn.Parameter(init_value * torch.ones((1, dim, 1, 1, 1)),requires_grad=True)
...
class EvoSABlock(nn.Module):
    ...
    def forward(self, cls_token, x):
        ...
        if self.prune_ratio == 1:
            ...
            fast_update = fast_update + self.gamma_1 * tmp_x[:, -1:]
            tmp_x = self.mlp(self.norm2(x))
            x = x + self.drop_path(self.gamma_2 * tmp_x)
            fast_update = fast_update + self.gamma_2 * tmp_x[:, -1:]
            # fast update
...
class Uniformer_light_ls_fp32(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        global init_value
        init_value = cfg.UNIFORMER.INIT_VALUE

...
class Hmdb(torch.utils.data.Dataset):
    """
    HMDB video loader. Construct the HMDB video loader,
    ...
    def __init__(self, cfg, mode, num_retries=10):
        """
        Load HMDB data (frame paths, labels, etc. ) to a given
        Dataset object. The dataset could be downloaded from HMDB
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for HMDB V1".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing HMDB {}...".format(mode))
    ...
    def __getitem__(self, index):
        ...
        start_idx, end_idx = get_start_end_idx(
            self._path_to_videos[index][1],
            clip_sz,
            temporal_sample_index,
            self.cfg.TEST.NUM_ENSEMBLE_VIEWS,
            use_offset=self.cfg.DATA.USE_OFFSET_SAMPLING,
        )
        seq = torch.linspace(start_idx, end_idx, num_frames)
        seq = torch.clamp(seq, 0, self._path_to_videos[index][1] - 1).long() + 1

...
class UniFormer(nn.Module):
    ...
    def __init__(self, depth=[3, 4, 8, 3], img_size=224, in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
                 ...
                 mix_token=False, return_dense=False, ):
        """
        Args:
            ...
            mix_token (bool): use mix token augmentation for batch of tokens
            return_dense (bool): whether to return feature of all tokens with an additional aux_head
        """
        super().__init__()
...
@register_model
def uniformer_small_dim64(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[3, 4, 8, 3],
...
@register_model
def uniformer_base_dim64(pretrained=True, **kwargs):
    model = UniFormer(
        depth=[5, 8, 20, 7],
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), 
...
@register_model
def uniformer_large_ls(pretrained=True, **kwargs):
    global layer_scale
    layer_scale = True
    model = UniFormer(
        depth=[5, 10, 24, 7],
        embed_dim=[128, 192, 448, 640], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), 
        return_dense=True, mix_token=True, **kwargs)
    model.default_cfg = _cfg()
    return model
...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, pos_embed_interp=True, num_patches=576, patch_hw=(16,12), patch_size=16, align_corners=False):
    if cfg is None:
    ...
    if pos_embed_interp:
        ...
        pos_embed_weight = F.interpolate(pos_embed_weight, size=patch_hw, mode='bilinear', align_corners=align_corners)
    ...
    _, _, patch_embed_proj_h, patch_embed_proj_w = state_dict['patch_embed.proj.weight'].shape
    if patch_embed_proj_h != patch_size or patch_embed_proj_w != patch_size:
        patch_embed_proj_weight = state_dict['patch_embed.proj.weight']
        patch_embed_proj_weight = F.interpolate(patch_embed_proj_weight, size=(patch_size, patch_size), mode='bilinear', align_corners=align_corners)
